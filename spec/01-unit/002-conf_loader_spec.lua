...
describe("Configuration loader", function()
  it("loads the defaults", function()
    ...
    assert.same({"0.0.0.0:8000", "0.0.0.0:8443 ssl", "127.0.0.1:9000 mock", "127.0.0.1:9443 ssl mock"}, conf.proxy_listen)
  ...
  it("loads a given file, with higher precedence", function()
    ...
    assert.same({"0.0.0.0:9000", "0.0.0.0:9443 ssl", "127.0.0.1:10000 mock", "127.0.0.1:10443 ssl mock"}, conf.proxy_listen)
    assert.is_nil(getmetatable(conf))
  end)
  ...
  it("extracts flags, ports and listen ips from proxy_listen/admin_listen", function()
    ...
    assert.equal("127.0.0.1", conf.mock_proxy_listeners[1].ip)
    assert.equal(9000, conf.mock_proxy_listeners[1].port)
    assert.equal(true, conf.mock_proxy_listeners[1].mock)
    assert.equal(false, conf.mock_proxy_listeners[1].ssl)
    assert.equal(false, conf.mock_proxy_listeners[1].http2)
    assert.equal("127.0.0.1:9000", conf.mock_proxy_listeners[1].listener)

    assert.equal("127.0.0.1", conf.mock_proxy_listeners[2].ip)
    assert.equal(9443, conf.mock_proxy_listeners[2].port)
    assert.equal(true, conf.mock_proxy_listeners[2].mock)
    assert.equal(true, conf.mock_proxy_listeners[2].ssl)
    assert.equal(false, conf.mock_proxy_listeners[2].http2)
    assert.equal("127.0.0.1:9443 ssl", conf.mock_proxy_listeners[2].listener)
  ...
  describe("validations", function()
    ...
    it("enforces listen addresses format", function()
      ...
      assert.is_nil(conf)
      assert.equal("proxy_listen must be of form: [off] | <ip>:<port> [ssl] [http2] [proxy_protocol] [mock], [... next entry ...]", err)
    end)
