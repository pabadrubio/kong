...
for _, strategy in helpers.each_strategy() do
  ...
  describe("Mock proxy [#" .. strategy .. "]", function()
    ...
    setup(function()
      ...
      mock_proxy_client = helpers.mock_proxy_client()
    end)

    teardown(function()
      if mock_proxy_client then mock_proxy_client:close() end
      helpers.stop_kong()
    end)
    ...
    it("returns results with global plugins", function()
      ...
      assert.equal("127.0.0.1:15555",    json.request.headers["Host"])
      assert.equal("127.0.0.1",          json.request.headers["X-Real-IP"])
      assert.equal("127.0.0.1",          json.request.headers["X-Forwarded-For"])
      assert.equal("route1.com",         json.request.headers["X-Forwarded-Host"])
      assert.equal("10000",              json.request.headers["X-Forwarded-Port"])
      assert.equal("http",               json.request.headers["X-Forwarded-Proto"])
      assert.same({},                    json.request.post)
      assert.same({},                    json.request.uri)
      assert.equal(60000,                json.balancer_address.connect_timeout)
      assert.equal(60000,                json.balancer_address.read_timeout)
      assert.equal(60000,                json.balancer_address.send_timeout)
      assert.equal(5,                    json.balancer_address.retries)
      assert.equal(1,                    json.balancer_address.try_count)
      assert.equal(15555,                json.balancer_address.port)
      assert.equal("ipv4",               json.balancer_address.type)
      assert.equal("127.0.0.1",          json.balancer_address.host)
      assert.equal("127.0.0.1",          json.balancer_address.hostname)
      assert.equal("127.0.0.1",          json.balancer_address.ip)
      assert.same({{
        balancer_latency = 0,
        ip               = "127.0.0.1",
        port             = 15555,
      }}, json.balancer_address.tries)
    end)
    ...
    it("returns results with global plugins with consumer specified", function()
      ...
      assert.equal("http",               json.request.headers["X-Forwarded-Proto"])
      assert.same({},                    json.request.post)
      assert.same(query_args,            json.request.uri)
      assert.equal(60000,                json.balancer_address.connect_timeout)
      assert.equal(60000,                json.balancer_address.read_timeout)
      assert.equal(60000,                json.balancer_address.send_timeout)
      assert.equal(5,                    json.balancer_address.retries)
    ...
    it("returns results with route specific plugins", function()
      ...
      assert.equal("127.0.0.1",          json.request.headers["X-Real-IP"])
      assert.equal("127.0.0.1",          json.request.headers["X-Forwarded-For"])
      assert.equal("route2.com",         json.request.headers["X-Forwarded-Host"])
      assert.equal("10000",              json.request.headers["X-Forwarded-Port"])
      assert.equal("http",               json.request.headers["X-Forwarded-Proto"])
      assert.same({},                    json.request.post)
      assert.same({},                    json.request.uri)
    ...
    it("returns results with service and route specific plugins with consumer specified", function()
      ...
      assert.equal("10000",              json.request.headers["X-Forwarded-Port"])
      assert.equal("http",               json.request.headers["X-Forwarded-Proto"])
      assert.same(post_args,             json.request.post)
      assert.same({},                    json.request.uri)
      assert.equal(60000,                json.balancer_address.connect_timeout)
      assert.equal(60000,                json.balancer_address.read_timeout)
      assert.equal(60000,                json.balancer_address.send_timeout)
    ...
    it("returns results with route and consumer specific plugins", function()
      ...
      assert.equal("127.0.0.1",          json.request.headers["X-Real-IP"])
      assert.equal("127.0.0.1",          json.request.headers["X-Forwarded-For"])
      assert.equal("route3.com",         json.request.headers["X-Forwarded-Host"])
      assert.equal("10000",              json.request.headers["X-Forwarded-Port"])
      assert.equal("http",               json.request.headers["X-Forwarded-Proto"])
      assert.same({},                    json.request.post)
      assert.same({},                    json.request.uri)
